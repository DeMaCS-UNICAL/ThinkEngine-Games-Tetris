...
spawnColumn(4).
myTile(R,C,V):-arenaTileSensor(T,objectIndex(Index),arenaTile(empty(V))), arenaTileSensor(T,objectIndex(Index),arenaTile(x(C))), arenaTileSensor(T,objectIndex(Index),arenaTile(y(R))).

numOfRows(R2):- #max{R1:myTile(R1,C,V)}=R, R2=R+1.
numOfColumns(C2):- #max{C1:myTile(R1,C1,V)}=C,C2=C+1.

rows(R1):- myTile(R1,_,_).
columns(C1):- myTile(_,C1,_).

startingConf(X,0):-tetrominoSpawnerSensor(tetrominoSpawner,objectIndex(_),tetrominoSpawner(lastInstantiated(X))).
bestSolution(X,Y,C) | a(X,Y,C):-columns(C),tetrominoConfigurationMaxWidth(X,Y,_),startingConf(X,_).
...
:-#count{R:bestRow(R)}=0.
:~ #count{R,C:hole(R,C)}=N,N1=3*N. [N1@4,N,N1]
:~ bestRow(R),numOfRows(N),D=N-R. [D@4,R,N,D]
:~ reach(R),numOfRows(N),D=N-R.  [D@3,R,N,D]
:~ bestSolution(X,Y,C). [C@2,X,Y,C]
:~ bestSolution(X,Y,C). [Y@1,X,Y,C]


setOnActuator(aIActuator(aI,objectIndex(Index),aIPlayer(aiProgressive(X)))) :- objectIndex(aIActuator, Index), tetrominoSpawnerSensor(tetrominoSpawner,objectIndex(_),tetrominoSpawner(progressiveNumber(X))).
setOnActuator(aIActuator(aI,objectIndex(Index),aIPlayer(numOfMove(X)))) :- objectIndex(aIActuator, Index), setOnActuator(aIActuator(aI,objectIndex(Index),aIPlayer(numOfLateralMove(N)))), setOnActuator(aIActuator(aI,objectIndex(Index),aIPlayer(numOfRotation(N1)))), X=N+N1.
setOnActuator(aIActuator(aI,objectIndex(Index),aIPlayer(numOfLateralMove(N)))) :- objectIndex(aIActuator, Index), bestSolution(X,Y,C),spawnColumn(S),leftSpaceWrtSpawn(X,Y,L), N=S-D,D=C+L,D<S.
setOnActuator(aIActuator(aI,objectIndex(Index),aIPlayer(numOfLateralMove(N)))) :- objectIndex(aIActuator,Index), bestSolution(X,Y,C),spawnColumn(S),leftSpaceWrtSpawn(X,Y,L), N=D-S,D=C+L,D>=S.
setOnActuator(aIActuator(aI,objectIndex(Index),aIPlayer(numOfRotation(N)))) :- objectIndex(aIActuator, Index), bestSolution(_,N,_).
setOnActuator(aIActuator(aI,objectIndex(Index),aIPlayer(typeOfLateralMove(left)))) :- objectIndex(aIActuator, Index), bestSolution(X,Y,C),spawnColumn(S),D=C+L,D<S,leftSpaceWrtSpawn(X,Y,L).
setOnActuator(aIActuator(aI,objectIndex(Index),aIPlayer(typeOfLateralMove(right)))) :- objectIndex(aIActuator, Index), bestSolution(X,Y,C), spawnColumn(S),D=C+L,D>=S,leftSpaceWrtSpawn(X,Y,L).
